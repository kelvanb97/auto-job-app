...
export function insertRole(
	...
	if (!role.url) return { status: "skipped" }
	...
	if (urlResult.data.length > 0) return { status: "duplicate" }
	...
	if (result.ok) return { status: "inserted", role: result.data }
